...
s = "WCETSTEEASTRYEOTHCATFGBTFLMOEYLHLIUEL"
arr = np.array(list(s))               # tableau de caractères
parts = np.array_split(arr, 3)        # 3 sous-tableaux
# afficher chaque partie comme chaîne
for i, p in enumerate(parts):
    print(i, ''.join(p.tolist()))
...
result = ''.join(out)
print("parts:", [''.join(p) for p in parts])
