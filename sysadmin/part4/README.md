ssh morpheus@challenges.ringzer0ctf.com -p 10146

Mot de passe: VNZDDLq2x9qXCzVdABbR1HOtz


Je me rappelais d'un dossier backup, dans lequel on avait quelques fichiers intéressant:
cat /backup/c074fa6ec17bb35e168366c43cf4cd19

ça contient une clé.
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
home/oracle/.ssh/id_rsa.pub0000640000175000017500000000061612310211305016600 0ustar  chamiltonchamiltonssh-rsa  oracle@forensics
home/oracle/.ssh/authorized_keys0000640000175000017500000000061612310211340017762 0ustar  chamiltonchamiltonssh-rsa  oracle@forensics


Bon j'ai mis la clé dans un fichier nommé key
J'ai quitté la connexion et depuis ma machine exécuté :
ssh oracle@challenges.ringzer0team.com -p 10146 -i key.pem


Ca fonctionne on est connecté et on voit un fichier flag.txt
cat flag.txt | base64 --decode
FLAG-d325e738fa7d87d4f5607c302b37db20